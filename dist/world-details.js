...
export function createWorldDetails(scene){
 ...
 const neon=groups.saber;
 for(let i=0;i<9;i++){
  const color=i%2?'#9777dd':'#6de7d6';const arch=new T.Mesh(new T.TorusGeometry(2.4,.017,6,48,Math.PI),material(color,true));arch.position.set(0,.08,-i*2.6);neon.add(arch);
 ...
 const targetRoom=groups.targets;add(targetRoom,box,'#20383b',[0,1.65,-4.7],[6.3,3.3,.12]);
 for(let i=0;i<5;i++){const ring=new T.Mesh(new T.TorusGeometry(.26,.035,8,40),material(i%2?'#e9b776':'#a8dcbc',true));ring.position.set((i-2)*1.05,1.7+Math.sin(i)*.4,-4.55);targetRoom.add(ring);add(targetRoom,sphere,'#d6e7bd',ring.position.toArray(),[.045,.045,.025],true);}
