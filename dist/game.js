...
import {sweptHit,strike,nextOpponent,punchExtension,punchHitsBody,BOXING_STOP_Z,zombieProfile} from './game-logic.js?v=demo-pass4';
import {ActivityTracker} from './activity.js';
import {createSound} from './sound.js?v=demo-pass4';
import {createImpactEffects} from './impact-effects.js?v=demo-pass4';
import {GAMES,createExtraGames} from './extra-games.js?v=demo-pass4';
import {TrajectoryPunchTracker,practiceStroke} from './motion.js?v=demo-pass4';
import {setupFullscreen} from './fullscreen.js';
import {createWorldDetails} from './world-details.js';
import {createAvatar,createLobbyAvatar} from './avatar.js?v=demo-pass4';
import {createBoxingModels} from './boxing-models.js?v=demo-pass4';
...
export function createArcade(renderer,getTracking){
 ...
 const enemies=[];let targetScore=0;let target=null,elapsed=0,nextSpawn=0,kills=0,hits=0,boxingScore=0,spawned=0,lives=5,wave=1,active=false,mode='zombies',state='menu',input='live',lastFrame=0,practiceStart=-10000,practiceClock=0,feedbackUntil=0,hudAt=0,practiceGuard=false,roundLimit=45;
 ...
 function finish(quiet=false,timed=false){sound.stop();if(!quiet)sound.play((mode==='zombies'?lives<=0:extraResult?.hp<=0)?'defeat':'finish');const score=extraResult?.score??(mode==='zombies'?boxingScore:targetScore);best[mode]=Math.max(best[mode]||0,score);try{localStorage.setItem('armature-bests',JSON.stringify(best))}catch{}state='over';hudAt=0;$('opponentHud').hidden=true;detector.reset();overlay(timed?'Round complete.':mode==='bird'?'Flight over.':mode==='zombies'?'Run complete.':(extraResult?.hp<=0?'Run complete.':'Time’s up.'),mode==='zombies'?`${kills} zombies defeated · ${hits} hits landed · wave ${wave}`:`${hits} targets hit in 45 seconds.`, 'Play again');$('overlayText').textContent=`Score ${score} · Best ${best[mode]} · ${mode==='sword'?`${extraResult?.parries||0} parries · ${extraResult?.finishes||0} finishes`:extraResult?extraResult.combo+(mode==='bird'?' gaps':' combo'):hits+' hits'}. Session: ${activity.kcal.toFixed(1)} estimated kcal.`;$('pauseGame').disabled=true;}
 ...
 if(state!=='paused'){roundLimit=Number($('roundDuration').value);clear();elapsed=0;nextSpawn=2.5;kills=hits=targetScore=boxingScore=spawned=0;lives=5;wave=1;extra.reset(mode);extraResult=null;if(mode==='zombies')zombie(-2.1);else if(mode==='targets')practiceTarget();}
 detector.reset();practiceStart=-10000;extra.group.visible=!['zombies','targets'].includes(mode);state='playing';hudAt=0;sound.setScene(mode,true,elapsed);$('viewport').focus({preventScroll:true});sound.play('bell');$('gameOverlay').hidden=true;$('pauseGame').disabled=false;$('pauseGame').textContent='Pause';}
 ...
 function practicePunch(){if(input==='practice'&&state==='playing'&&practiceClock-practiceStart>(mode==='bird'?.23:.8)){action=true;practiceGuard=false;$('practiceGuard').setAttribute('aria-pressed','false');practiceStart=practiceClock;if(mode!=='bird')sound.play(['sword','saber'].includes(mode)?'slashSwing':'whoosh',{power:.5+Number($('strikeEffort').value)*.25,pan:.3});}}
