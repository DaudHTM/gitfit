...
import {sweptHit} from './game-logic.js?v=controls-fix3';
import {FlapTracker,FLIGHT,gripQuaternion} from './motion.js?v=controls-fix3';
import {FlightCourse} from './flight-logic.js?v=controls-fix3';
export const GAMES={
 ...
 sword:{name:'Sword Arena',title:'BLADE / ARENA',tag:'SWORD SURVIVAL',intro:'Hold your ground.',help:'Slash through a knight’s armor, then swing again to finish. Watch their attack wind-up. Recover between swings. Space swings in practice.'},
...
export function createExtraGames(scene,mesh,geo,feedback,sound){
 ...
 function tick(dt,fist,q,practice,action,samples=[]){
  ...
  if(time>=next){spawn();next=time+(mode==='saber'?1:mode==='sword'?Math.max(1.8,3.5-score/2000):Math.max(.8,1.8-score/3000));if(mode==='saber')sound.play('beat');}
