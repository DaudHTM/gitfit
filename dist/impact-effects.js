...
const COLORS={punch:'#ffd19b',knockout:'#ffe5ae',armor:'#d8e7ff',slash:'#8fffe5',block:'#88dfff',ring:'#ffdc79',target:'#c8ffa0',hurt:'#de785a',miss:'#c2c6c9'};
...
export function createImpactEffects(scene){
 ...
 const particles=Array.from({length:count},()=>({p:new T.Vector3(),v:new T.Vector3(),life:0,total:1,size:.01,spin:0}));let cursor=0,ringCursor=0,arcCursor=0,enabled=true;
 ...
 function burst(point,kind='punch',power=1,direction=null){
  ...
  shards.instanceColor.needsUpdate=true;ring(p,kind,power);
  if(kind==='slash'||kind==='armor')ring(p,kind,power,true,direction?Math.atan2(direction.y,direction.x):-.5);
  if(kind==='knockout')ring(p,kind,power*1.6);
 ...
 function clear(){shards.visible=false;for(const p of particles)p.life=0;for(const r of [...rings,...arcs]){r.life=0;r.mesh.visible=false;}flash.intensity=0;glow=0;vignette.style.opacity='0';streaks.visible=false;}
 function tick(dt,camera,{effects=true,flight=false,speed=0,paused=false}={}){
  ...
  let alive=0;for(let i=0;i<count;i++){const p=particles[i];p.life=Math.max(0,p.life-dt);if(p.life){alive++;p.v.y-=dt*1.6;p.p.addScaledVector(p.v,dt);p.v.multiplyScalar(Math.exp(-dt*2));dummy.position.copy(p.p);dummy.rotation.set(p.spin+p.life*4,p.spin,p.life*5);dummy.scale.set(p.size,p.size*.5,p.size*(p.life/p.total));}else dummy.scale.setScalar(0);dummy.updateMatrix();shards.setMatrixAt(i,dummy.matrix);}shards.visible=alive>0;shards.instanceMatrix.needsUpdate=true;
