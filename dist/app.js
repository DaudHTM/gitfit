...
import {SERVICE,DATA,CONTROL,HEART,decode,sequenceGap,decodeHeart,heartDisplay} from './protocol.js';
...
let device=null,control=null,stream=null,busy=false,demo=false,last=0,seq=null,lost=0,count=0,rateStart=performance.now(),latestFlags=0,calPending=false,calDeadline=0;
let heartStream=null,heartPacket=null,heartLast=0,heartAvailability='disconnected',heartUi=0;
function updateHeart(now){const state=heartDisplay(heartPacket,heartLast,now,heartAvailability);$('heartValue').textContent=state.value;$('heartStatus').textContent=state.status;$('hudHeart').textContent=state.value+' BPM';$('heartPanel').classList.toggle('has-pulse',state.valid);}
function receiveHeart(e){try{heartPacket=decodeHeart(e.target.value);heartLast=performance.now();heartAvailability='available';}catch{heartPacket=null;heartAvailability='error';}}
function message(s){$('message').textContent=s}
function reset(){heartStream=null;heartPacket=null;heartLast=0;heartAvailability='disconnected';updateHeart(performance.now());control=null;stream=null;busy=false;last=0;seq=null;count=0;lost=0;latestFlags=0;calPending=false;$('connect').disabled=false;$('connect').textContent='Connect ESP32 ↗';$('connection').textContent='Disconnected';$('mode').textContent='PREVIEW';$('calibrate').disabled=true;$('calStatus').textContent='Connect to calibrate · hold for 3 seconds';$('upperState').textContent=$('lowerState').textContent='Offline';$('rate').textContent=$('age').textContent=$('lost').textContent='—';$('demo').disabled=false;}
function receive(e){try{const p=decode(e.target.value);const now=performance.now();lost+=sequenceGap(seq,p.sequence);seq=p.sequence;last=now;count++;latestFlags=p.flags;
 ...
 if(calibrating)calPending=false;
 $('calibrate').disabled=calibrating||calPending||fault;
 $('calStatus').textContent=fault?'Sensor read error — check wiring':calibrating?`Hold still… ${p.progress}%`:calPending?'Waiting for ESP32…':(p.flags&8)?'Movement detected. Hold the pose and retry.':ready?'Reference set. You can move.':'Hold the pose, then calibrate.';
 $('mode').textContent=fault?'SENSOR ERROR':calibrating?'CALIBRATING':ready?'LIVE':'NEEDS CALIBRATION';$('upperState').textContent=$('lowerState').textContent=fault?'Check wiring':'Streaming';
...
heartPacket=null;heartLast=0;heartAvailability='waiting';
try{heartStream=await service.getCharacteristic(HEART);heartStream.addEventListener('characteristicvaluechanged',receiveHeart);await heartStream.startNotifications();}catch(error){heartAvailability=error.name==='NotFoundError'?'unsupported':'error';}
last=0;seq=null;lost=0;count=0;rateStart=performance.now();$('connection').textContent='ESP32 connected';$('connect').textContent='Disconnect';$('demo').disabled=true;$('calibrate').disabled=false;message('Hold your arm straight down, palm toward your thigh. Calibrate before moving.');}catch(err){device?.gatt?.disconnect();reset();message(`Connection failed: ${err.message}`)}finally{busy=false;$('connect').disabled=false}};
...
renderer.setAnimationLoop(now=>{if(now-heartUi>=250){heartUi=now;updateHeart(now);}if(demo&&!arcade.active){qu.setFromEuler(new THREE.Euler(.15*Math.sin(now/1300),0,.32+.25*Math.sin(now/1900)));qf.copy(qu).multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1,0,0),-.85-.8*Math.sin(now/1100)))}
