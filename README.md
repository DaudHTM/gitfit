...
The home page has **Zombie Boxing**, **Bird Flight**, **Neon Saber**, **Sword Arena**, **Orbit Guard**, **Target Rush**, and **Motion Lab**. Connection and calibration are shared across modes; switching games keeps the BLE connection. The two arm quaternions keep the same 20-byte packet layout. Updated flags and commands support two-pose calibration.
...
Use Fullscreen in the bottom-right of the scene. Exit, pause/resume and practice strike/flap controls stay inside the fullscreen view. When native fullscreen is unavailable or stalls in an embedded browser, the game immediately expands to fill the page instead. Escape exits this expanded fallback and pauses. These website changes use the existing firmware and need no ESP32 reflash.
