...
The home page has **Zombie Boxing**, **Bird Flight**, **Neon Saber**, **Sword Arena**, **Spell Rush**, **Orbit Guard**, **Target Rush**, with a live avatar in the lobby. Connection and calibration are shared across modes; switching games keeps the BLE connection. The two arm quaternions keep the same 20-byte packet layout. Updated flags and commands support two-pose calibration.
...
- Spell Rush: jab for Firebolt, hook for Arc, uppercut for Rise. Match the highlighted ward before it reaches you. The first three enemies teach the gestures; later ones need two different spells. Every five defeated enemies activates six seconds of double points. Curved spell trails, shattering wards, a crystal portal world and a distinct soundtrack accompany each cast. Practice supports Punch + Cast or 1 / 2 / 3. Results show match accuracy and best chain.
- Sword Arena: cross blades to parry the incoming sword, then make a fresh slash during the opening for a one-hit counterattack. The first parry on an opponent earns 25 points; finishes award 200–300 plus combo bonuses. Guarded armor blocks direct attacks. Three opponents have distinct wind-up, swing and opening timings.
- Target Rush: hit as many illuminated targets as possible in 45 seconds.
- Bird Flight: a Flappy Bird-style survival course. Forward motion stays at 4 m/s; flaps only change altitude. Fly between the upper and lower obstacles. Clear a whole gap for 100 points; an obstacle or ground collision ends the run.
- Each round has a compact challenge: five zombie defeats, five flight gaps, an eight-slice chain, three sword finishes, five shattered wards, an eight-block chain, or twelve targets. A short chime confirms completion. Results show a large score, three game-specific stats, challenge progress and round calorie estimates; Play again and Next game keep the demo moving without reconnecting the arm.
- Every game has its own original background score and layered action sounds. Boxing uses heavy percussion and punch/knockout layers; sword combat has metal clashes; Neon Saber runs at 120 BPM with a cut every second beat; flight pairs a quieter score with wind and wingbeats. Shield blocks, target hits and cleared gaps have their own cues.
...
Choose Zombie Boxing (speed-based damage), Target Rush (45 seconds), Bird Flight (vertical obstacle survival), Neon Saber (original rhythm slicing), Sword Arena (parry and counter), Spell Rush (gesture magic), or Orbit Guard (intercept meteors). Spell Rush recognizes the same wrist trajectories as boxing; it targets the front ward automatically so the demo focuses on the gesture. The same calibrated right-arm connection stays active when switching games. Bird Flight uses downward wrist strokes for lift while moving forward automatically; Orbit Guard follows hand position. Neon Saber and Sword Arena use moving blade collisions.
...
Neon Saber and Sword Arena attach the blade rigidly at a 90-degree grip angle to the forearm. Their collision checks follow the same rendered blade. Sword and saber collisions now process intermediate sensor blade poses between render frames, retaining up to 100 ms / 12 samples, rejecting stale jumps and duplicate timestamps. Displayed arm orientation still uses the newest pose immediately; no playback queue is added. Neon Saber adds trails, split-block effects and an advance cut cue. Its practice animation cuts across the strike plane, then recovers before the next beat; Sword Arena gives enemies visible swords, guarded armor, readable attack wind-ups and a timed parry/counter loop. Hold your blade across the incoming sword to stagger the opponent; a new slash during the opening finishes them. Practice: hold G or toggle Guard, then press Space or Strike to counter. Missed parries cost health; missed openings restore the opponent’s guard. Target Rush awards accuracy and reaction-time bonuses. Orbit Guard uses detailed moving meteors and growing combos.
...
Keep Round set to Quick (45 seconds). Connect the actual arm and complete both calibration poses before the presentation. Sword Arena demonstrates a visible blade parry and a one-hit counter within its first few seconds. Boxing introduces runners and a three-hit-or-more brute early. Spell Rush demonstrates jab / hook / uppercut recognition as three visibly different spells; match the gesture above the approaching ward. Use Play again or Lobby between demonstrations; the Bluetooth connection stays shared. Practice is visibly labeled and never counts calories. Assets and music are local once the app is served, so engine/font CDN availability is not part of startup.
