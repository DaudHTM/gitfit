...
test('gaps score only after fully clearing the obstacle and never twice',()=>{
 const c=new FlightCourse();c.gates=[flightGate(0,-1),flightGate(1,-20)];
...
test('a collision ends flight once, freezes scoring, and reset starts a fresh course',()=>{
 const c=new FlightCourse();c.gates=[flightGate(0,-1),flightGate(1,-20)];c.flight.y=1;
 ...
 let crash;for(let i=0;i<200&&!crash;i++)crash=c.step(.01).find(e=>e.type==='crash');assert.equal(crash.reason,'ground');
});
test('the sky limit clamps height without an invisible ceiling collision',()=>{
 const c=new FlightCourse();c.flight.y=5.7;c.flight.vy=3;
