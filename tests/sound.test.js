...
test('all action cues construct finite, bounded envelopes',async()=>{
 ...
 for(const kind of ['punch','knockout','armor','slash','slashSwing','block','hurt','miss','flap','step','growl','target','ring','bell','beat','finish','defeat','select']){f.sound.play(kind,{power:1.6,pan:-.7});assert.ok(f.sound.status.voices>0);f.ctx.advance(2);assert.equal(f.sound.status.voices,0);}
});
